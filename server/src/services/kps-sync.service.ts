...
export const syncGokupsKps = async (logger: SyncLogger = defaultLogger): Promise<KpsSyncResult> => {
  const metaPage = await fetchPage(1)
  const firstPage = START_PAGE === 1 ? metaPage : await fetchPage(START_PAGE)
  const seenIds = new Set<string>()
  let processedRows = 0

  logger.info(`GoKUPS total rows: ${metaPage.total}, last_page=${metaPage.lastPage}, start_page=${START_PAGE}`)
